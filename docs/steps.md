...
因此主插件應專注於：比賽狀態、玩家/隊伍、世界建立與預生成、邊界、傳送、勝負、可配置規則、必要主持工具。DiscordSRV、封包音效控制、自訂礦物插件、舊版附魔模擬、舊 WorldBorder 插件實作、GUI、統計展示、練習模式等，不能再以舊式 hard dependency 和核心生命週期綁死；但舊版已提供且被確認仍要保留的功能，必須透過原生 API、選配 integration 或後續專用實作完整保留。
...
3. 沒安裝 `PacketListenerAPI`、PacketEvents、舊外部 WorldBorder 插件、custom-ore-generator、DiscordSRV 時，核心 UHC 仍可啟動並跑完最小流程；邊界與預生成不得因此缺席，若改採新外部替代插件，該插件必須列入升級後部署條件並通過驗證。`custom-ore-generator` 與 DiscordSRV 的語意沿用舊版：有安裝且設定啟用時要完整提供功能，未安裝時只停用對應整合。
...
- 核心 UHC 比賽流程可以在沒有 DatouNMS、PacketListenerAPI/PacketEvents、custom-ore-generator、舊外部 WorldBorder 插件、DiscordSRV 的情況下運作；邊界與預生成能力必須由原生 API、已驗證替代插件或專用實作完整保留。custom-ore-generator 與 DiscordSRV 功能則採舊版選配模式：未安裝不阻塞核心，有安裝就必須完整接上。
...
| --- | --- | --- | --- |
| `PacketListenerAPI` | `WonderlandUHC.checkDepends()` 強制檢查；`PacketRegister` 註冊 `SoundController`；`SoundController` 攔音效與攻擊封包；`ScenarioArmorVsHealth` 攔 `PacketPlayOutUpdateAttributes` 更新血量。 | 不整合舊 `PacketListenerAPI`，也不預設導入 PacketEvents。現有 packet 用途不是 UHC 開局、隊伍、死亡、勝負的核心條件；能用 Paper/Bukkit 原生事件與 Attribute API 取代的先取代，不能穩定取代且非核心的功能先降級或移除。 | 先移除 `Dependency.PACKET_LISTENER_API.check()` 的硬阻塞。`ArmorVsHealth` 優先用 Paper/Bukkit equipment/attribute 事件改寫；死亡/旁觀互動限制優先用 Bukkit event gate。`SoundController` 不直接移植，除非後續確認該音效控制是必留需求，才評估 optional PacketEvents adapter。 |
| 外部 `WorldBorder` 插件 | `WonderlandUHC.checkDepends()` 強制檢查；`checkWorldBorderVer()` 綁外部版本；`BorderUtil` 直接 dispatch `wb clear/set/wshape`；`ChunkFiller` 直接跑 `wb fill`；`WorldFillListener` 依賴 `WorldBorderFillFinishedEvent`。`Bedrock_Border_Height` 舊設定預設為 `0`，只有大於 `0` 時才會在 fill 完成後額外生成基岩牆。 | 不整合整個舊 WorldBorder 插件到本體。邊界與預生成是必留能力，但實作來源不預設為本體手刻；應先驗證 `WorldBorder [Renew] 1.18.X - 1.21.X`、`Chunky` 等新版本替代插件能否完整承接 border / fill / trim / progress / completion workflow。基岩牆不是原生邊界核心，僅作舊設定相容。 | 拆成 `BorderService`、`ChunkPregenerationService` 作為主要接線邊界。Paper/Bukkit 原生 `WorldBorder` 可承接邊界設定與收縮；fill/trim/progress/完成事件優先找新版本外部插件 adapter。`BedrockBorderService` 只在 `Bedrock_Border_Height > 0` 時作 legacy/optional compatibility，不作核心 blocker。完整手刻只作最後手段。若邊界、預生成或舊設定啟用時的必要行為未完整保留，本步驟不能標記完成。 |
| `custom-ore-generator` | `WorldInitListener` 在 UHC 世界初始化時套用；`CustomOreGeneratorHook` 從 Bukkit services 取 API provider；`Populator`/`OreGen` 直接使用 COG API；`GeneratorSettingsMenu` 顯示 populator 設定。 | 不整合進主插件核心，但沿用舊版語意：有安裝就啟用自訂礦物 / populator，沒有安裝就無視，不阻塞核心 UHC。已查到原 Custom Ore Generator 有標示支援 1.21 系列，另有 `Ore Control` 等同作者替代候選；因此優先驗證新版 COG，其次才評估替代插件，不預設自製。 | 建立 `OreGenerationPort`、`NoopOreGenerationPort`、`CustomOreGeneratorAdapter`。先驗證新版 COG API / service provider / 指令與 1.21.11 相容；若不合適，再驗證 `Ore Control` 或其他插件。`populators.yml` 必須遷移到 1.18+ 高度、deepslate/raw ore 與新版 replace materials。 |
...
- Practice mode
- 舊版附魔模擬
- 基岩牆 / 方塊假邊界視覺效果；只作舊設定相容，預設停用時不列為核心 blocker
...
- `Update-WonderlandUHC/src/main/java/org/mcwonderland/uhc/menu/impl/PlayersMenu.java` 目前仍透過 Foundation `ItemCreator.of(CompMaterial)` 建立 player head；等 Step 12 menu adapter/view model 收斂時再處理。
- `Update-WonderlandUHC/src/main/java/org/mcwonderland/uhc/menu/ButtonLocalization.java` 目前被 Foundation `ButtonReturnBack.setMaterial(CompMaterial)` 簽名綁住；等 Step 12 替換或包住 Foundation menu API 時再處理。
- `Update-WonderlandUHC/src/main/java/org/mcwonderland/uhc/scenario/impl/ScenarioConfig.java` 目前同時被 Foundation `YamlConfig#getMaterial` 與 `ItemCreator.of(CompMaterial, ...)` 綁住；等 Step 8 設定/material migration 與 Step 12 scenario menu 顯示收斂時再處理。
...
1. 新增 `PlatformCapabilities`，明確列出 absorption、fast block set、custom exp orb、old enchant、death animation、pickup exp control、large chest merge 是否可用。
...
5. 無穩定 API 的功能不可直接刪除；例如 old enchant hack、death animation、pickup exp control、自訂 exp orb、fast block set，必須先判斷是否屬於舊版對外可見且需完整保留的行為。若暫時做不到，只能標成未完成或需使用者確認的降級項，不能把停用視為完成。
6. `ScenarioTimeBomb` 目前透過 `NewerSpigotAPI.mergeChest()` 使用 DatouNMS model helper；本步驟要一併收斂到 adapter 或改用 Paper/Bukkit 可維護做法。
7. `TestCommand` 若只剩人工測試用途，優先移除或改成不依賴 DatouNMS；不要為測試指令保留核心 runtime 依賴。
...
- 不為了消除 import 而擴大重構 scenario 架構；scenario feature gate 與規則引擎整理留到 Step 11。
...
- DatouNMS 不可用時，插件仍可啟動，核心 UHC 最小流程不因 old enchant、death animation、pickup exp control、fast block set 或 custom exp orb 缺席而中止。
...
2. **確認 WorldBorder 與預生成替代路線**
   ...
   - `wb fill`、trim、progress、完成事件不是原生 `WorldBorder` API 的完整能力；先驗證 `WorldBorder [Renew] 1.18.X - 1.21.X` 是否可替代舊 WorldBorder 插件。
   - 另行驗證 `Chunky` 是否可作為預生成 / trim 的替代插件，包含 square selection、worldborder sync、progress、完成偵測與 existing world loading 流程銜接。
...
3. **移除舊 Packet 依賴，優先改用原生 API**
   ...
   - `SoundController` 不直接移植；目前音效判斷本來就不完整，除非後續確認為必留需求，才評估 optional PacketEvents adapter。
...
4. **自訂礦物沿用舊版選配語意**
   ...
   - 優先驗證新版 Custom Ore Generator 是否能在 Paper `1.21.11` 承接舊版 `populators.yml` / service provider / world init 工作流；若不能，再驗證 `Ore Control`、`DevWorldGen` 等替代插件。
   - 有安裝並啟用自訂礦物插件時，UHC 世界應套用選定 generator / populator；沒有安裝時，流程必須和舊版一致地無視此功能，不阻塞核心開局。
...
6. **DiscordSRV 完整保留但解除硬綁定**
   ...
   - DiscordSRV integration 至少要覆蓋：賽事公告、頻道設定、提及轉換、隊伍語音頻道建立、入隊/退隊搬移、死亡/離隊回 lobby、`/reconnect`。
   - DiscordSRV 未安裝時只停用 Discord 功能，不影響核心開局、死亡、勝負或結束流程；有安裝但設定錯誤時要明確回報，不可靜默失敗。

完成條件：

- 空 Paper 1.21.11 server 可啟動核心 WonderlandUHC。
- 舊 WorldBorder 插件被移除後，邊界、fill/trim、預生成進度與完成後續流程仍需由原生 API 或已驗證替代插件完整保留；不能把「跳過預生成」視為完成。
- 自訂礦物沿用舊版語意：沒安裝時核心流程可繼續，有安裝時可套用新版驗證過的 COG 或替代插件，並能正確處理 1.21.11 礦物高度與材料。
- DiscordSRV 沒安裝時只停用整合；安裝並啟用時，公告與隊伍語音流程完整可用。
...
- `Update-WonderlandUHC/src/main/resources/gamecache.db`（若確認仍未被程式引用，視為舊殘留資源，不遷移）
...
4. `cache.db` 只保留會影響切階段 / 重啟接續的資料，例如 `Loading_Status`、host、已選 settings；這些讀寫要從 core 分離，透過 `StoragePort`。
5. `savedgames.db` 保留舊版主持設定 preset 或等價功能；`gamecache.db` 若確認仍未被程式引用，視為舊殘留資源，不實作、不遷移。
6. 生成狀態、host、已選設定與世界載入狀態必須可持久化；生成完成後要像舊版一樣保存 `DONE` 或等價狀態，重啟後不能回到未生成或設定遺失。
7. `StatsStorageSql` / MySQL 長期資料庫路線不保留；stats 改成 YAML 或等價本機儲存，至少能正確紀錄當局資料，並保留 `/stats`、`/topkills`、`StatsMenu` 等既有查詢 / 顯示入口。
8. 設定讀取錯誤應明確回報，不應在任意 class static init 時爆炸。
9. `ScenarioConfig` 的 `Type` 讀取與 scenario icon material 應從 Foundation `CompMaterial` 過渡到 Bukkit/Paper 原生 material key；需要和 scenario resource migration 一起做，不在 Step 5 單點硬改。

完成條件：

- 核心 use case 可以透過 settings object 測試。
- 舊設定升級前會備份，升級後可讀。
- 生成完成狀態、host 與 UHC 設定在重啟後仍能恢復。
- 未被程式引用的舊殘留資料檔不得被列為必做 migration；目前看似未使用的 `gamecache.db` 要先確認引用再決定是否保留。
- MySQL stats 設定已移除或明確標成不支援；YAML / 本機 stats 可正確保存與讀取當局資料。
...
Scenario 是 UHC 的重要玩法，舊版既有 scenario 都要保留功能；但不需要焊死在舊的實作方式，也不應讓每個 scenario 任意操作全域狀態、NMS、外部插件。
...
3. NMS/packet 型 scenario 先獨立 feature gate，例如 `ScenarioArmorVsHealth`；優先用原生 API 或穩定替代插件重做同等功能，不能因舊 packet/NMS 不可用就刪除 scenario。
...
7. 建立逐項 scenario 驗收表：舊版行為、1.21.11 實作方式、設定 migration、測試方式、是否有可接受差異。

完成條件：

- 可以只載入核心 rule set 跑遊戲。
- 任一 scenario 壞掉時，不影響其他 scenario 與核心流程。
- 舊版預設註冊的 scenario 都有保留策略與驗收結果；除非使用者明確接受，不能移除或降級。
...
5. Practice mode 保留 `/practice` 舊功能，但可用更原生、更簡潔的實作方式重做；它不應阻塞核心啟動，但指令、練習世界、邊界、物品、隨機傳送與死亡補裝要維持同等行為。
6. Foundation command/menu 先用 adapter 包住，之後逐步替換。
7. 主持工具是核心操作入口，不是 UI 美化；`/uhc choose/start/stop/regen/tp/edit`、`/border`、白名單、staff、respawn、giveall 等必須逐項驗收功能不變。
8. `PlayersMenu`、`ButtonLocalization` 這類仍被 Foundation `ItemCreator` / `ButtonReturnBack` API 綁住的 material 使用，應在 menu adapter 或替代 menu builder 落地時一起收斂，不在 Step 5 用 `ItemStack` 繞路消除 import。

完成條件：

- 任何核心流程都能不用 GUI 完成。
- GUI 壞掉不會阻塞主持人用指令開局。
- Practice 與主持工具都有指令級驗收清單，升級後行為與舊版等價或差異已被明確接受。
...
2. 對同一份 checklist 分別在 Paper `1.16.5` 與 Paper `1.21.11` 執行，至少覆蓋：開服啟動、主要 command、host 設定、玩家加入、隊伍、scenario、倒數、傳送、開局、死亡、觀戰、復活、掉落、經驗、邊界、世界生成、勝負判斷、結束流程、GUI、scoreboard、optional integrations。
3. 每個項目必須分類為「一致」、「可接受差異」、「版本限制造成的差異」、「升級造成 regression」、「原版既有問題」。
4. 所有「升級造成 regression」必須修正，或取得明確接受紀錄後才能進入發布步驟。
5. 對 Step 5 到 Step 13 曾改動或降級的 legacy/platform/integration 行為要特別標註，例如 absorption、armor points、pickup exp control、death animation、custom exp orb、old enchant、fast block set、large chest merge、WorldBorder、Packet、custom-ore-generator。

完成條件：

- core/application 不需要啟動 Bukkit 就能測。
- Paper smoke test 可驗證插件啟動與最小遊戲流程。
- 1.16.5 / 1.21.11 功能對照檢核已完成，所有 regression 都已修正或被明確接受。
...
15. 補測試、1.16.5 功能對照檢核、CI、文件、發布流程。
...
目前專案使用 `PacketListenerAPI_v3.7.9-SNAPSHOT.jar`，這不是 1.21.11 可靠基礎。掃描後確認目前 packet 用途集中在音效控制與 `ArmorVsHealth` scenario，不是 UHC 開局、隊伍、死亡、勝負的核心必需能力。Step 7 預設方向是移除舊 Packet 依賴並優先使用 Paper/Bukkit 原生 API；PacketEvents 只在後續確認有原生 API 做不到、且必須保留的需求時才作 optional adapter。
...
自訂礦物沿用舊版選配語意：有安裝並設定使用時套用，沒有安裝時無視，不阻塞核心世界建立。優先驗證新版 Custom Ore Generator 是否能在 Paper `1.21.11` 承接舊版功能；若不合適，再驗證 `Ore Control`、`DevWorldGen` 等替代插件。不要預設自製礦物生成器，也不要把完整外掛整合進主插件。
...
4. 先驗證新版 COG：Paper `1.21.11` 載入、Bukkit service provider、舊版 API 等價性、每世界設定、只影響新 chunk、與預生成流程的套用時機。
5. 若新版 COG 不適合，再驗證替代插件：至少包含 `Ore Control` 與 `DevWorldGen`，並比較是否能支援 UHC 需要的 preset、每世界啟用、材料與高度規則。
6. `populators.yml` 要做 1.18+ 資料模型 migration：支援 `min_y=-64`、`max_y=320`、deepslate/raw ore、replace materials 不只 `STONE`，並明確分 overworld/nether/end。
7. `GeneratorSettingsMenu` 與 `MainSettingsMenu` 在 ore service 不可用時顯示停用狀態，不能 throw 或打開空資料。
8. `WorldUtils.getBlockEXP()`、礦物掉落、exp 與 scenario 若有礦物假設，要一起調整到 1.21.11 材料名稱。
9. 只有在已確認沒有可接受插件替代時，才討論內建 UHC 專用 ore rule service；這是最後手段，不是預設規劃。

### A6. WorldBorder 與 Chunk Fill

核心邊界可優先改用 Bukkit/Paper `WorldBorder`，但預生成 / fill / trim / progress / 完成事件要先尋找新版本插件替代。外部 WorldBorder 插件的主要價值在舊式 fill/trim 工作流，不是比賽邊界本身；這部分不能因原生 API 不完整就刪除。Paper 1.21.11 的 `WorldBorder` API 已提供 center、size、warning、damage 與 `changeSize`，其中舊的 `setSize(double, long)` 路線已被標記 deprecated，升級時要避免把新程式寫在即將移除的 API 上。
...
3. 驗證 `WorldBorder [Renew] 1.18.X - 1.21.X` 是否能替代舊 `wb` 指令、fill、trim 與完成事件；若可行，建立 `integration/worldborder/` adapter，而不是手刻整個插件。
4. 驗證 `Chunky` 是否更適合作為預生成 / trim 實作；若採用 Chunky，文件要明確寫出指令流程、進度判斷、完成偵測、overworld/nether 順序與重啟銜接。
5. `BorderUtil.setWBBorder()`、`removeWBBorder()`、`ChunkFiller.fill()`、`WorldFillListener` 不得在替代方案確認前直接刪除；遷移後的行為必須覆蓋舊版既有功能。
6. `WorldBorderFillFinishedEvent` 不能留在核心 listener；若替代插件有事件或 API，放在 `integration/` adapter，若只有 command/progress，必須有明確輪詢或完成偵測策略。
7. 基岩牆不是原生邊界核心；`Bedrock_Border_Height` 舊設定保留相容。預設 `0` 時不生成、不作核心 blocker；若設定大於 `0`，才透過 legacy/optional service 實作或明確回報目前不支援，不能默默移除啟用後的舊行為。
8. `checkWorldBorderVer()` 與 `Dependency.WORLD_BORDER.check()` 刪除或改成新依賴驗證，不可阻止主插件用錯誤訊息以外的方式硬崩。
9. smoke test 必須覆蓋選定替代方案：插件啟動、設定邊界、預生成、進度/完成偵測、開始收縮、停止遊戲都可運作。
...
- 缺少 custom-ore-generator、DiscordSRV、PacketListenerAPI/PacketEvents 時，只停用對應整合；若安裝 custom-ore-generator 或 DiscordSRV 並啟用設定，舊版等價功能必須完整可用。
