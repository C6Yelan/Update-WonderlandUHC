...
5. 無穩定 API 或非核心的功能先降級或停用，例如 old enchant hack、death animation、pickup exp control、自訂 exp orb、fast block set。停用時應記錄清楚原因，但不可阻塞核心開局、死亡、觀戰、傳送、勝負與結束流程。
...
6. `ScenarioConfig` 的 `Type` 讀取與 scenario icon material 應從 Foundation `CompMaterial` 過渡到 Bukkit/Paper 原生 material key；需要和 scenario resource migration 一起做，不在 Step 5 單點硬改。
...
7. `PlayersMenu`、`ButtonLocalization` 這類仍被 Foundation `ItemCreator` / `ButtonReturnBack` API 綁住的 material 使用，應在 menu adapter 或替代 menu builder 落地時一起收斂，不在 Step 5 用 `ItemStack` 繞路消除 import。
