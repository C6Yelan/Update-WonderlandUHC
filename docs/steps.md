...
- `/uhc edit`、`/uhc sethost`、`/setspawn`、白名單、MLG、reload/tutorial 與一般 host setting menu：目前沒有足夠複雜度支撐新的抽象。
- `/giveall`：功能正常，且單一入口的 CombatRelog 給物品流程與 Foundation 移除主線關聯不足，不為了瘦身硬抽。
