...
- 不處理 `gui.yml`、`items.yml`、`sounds.yml`、`biomes.yml` 的全量 alias 寫回；menu 與 presentation resource material 收斂留到 Step 12。
...
- Slice 7 收尾清理已完成：舊版 CenterCleaner pass/fail 失敗訊息 Java 欄位與預設 key 已移除；舊演算法設定 Java 欄位已移除，但預設 `settings.yml` 欄位保留為 legacy 相容欄位；`biomes.yml` 目前只剩資源抽出，保留到 Step 12 / resource migration 再處理。
- 細部地形/水域門檻調整延後到整體升級完成後再回來處理；不要在 Slice 7 混入評分演算法變更。
- Step 9 本輪升級可視為暫時完成：中心搜尋、MatchCenter 保存、WorldBorder / pregeneration / border 判斷、回中心與散佈流程已接到目前中心；spectator tool、Nether GUI 開關與 Nether portal 實跑仍需等 Step 10 修核心流程或 GUI 後回測。
...
7. 修核心流程或 GUI / spectator hotbar 後，必須回測 Step 9 Slice 7 目前 blocked 的項目：spectator center tool 是否傳送到 `MatchCenter`、Nether 開關是否能啟動、Nether 進出座標是否以主世界 `MatchCenter / 8` 作為 Nether center。
...
- 修復 GUI / spectator hotbar 或 Nether 開關後，已回頭驗收 Step 9 Slice 7 的 spectator tool 與 Nether 對應座標，不得只做啟動測試。
...
9. `gui.yml`、`items.yml` 與 menu button material alias 仍要做 1.21 名稱收斂；這不是 Step 8 的 storage 邊界，應跟 menu adapter / 替代 menu builder 一起處理。
10. 已知 Paper `1.21.11` 風險：`ScoreBoardUpdater -> ScoreLines -> StaffLines -> RuntimeUtil.getTPS()` 原本會嘗試反射舊 NMS TPS 欄位，實測會噴 `ClassNotFoundException: net/minecraft/server//MinecraftServer` 與連續 NPE。已先補 stopgap：優先讀 Paper `getTPS()`，不可用時回傳 `20.0` 且不刷 stacktrace；Step 12 仍應把 TPS 顯示整理成正式 scoreboard metric adapter。
...
5. 對 Step 5 到 Step 13 曾改動或降級的 legacy/platform/integration 行為要特別標註，例如 absorption、armor points、pickup exp control、death animation、custom exp orb、fast block set、large chest merge、WorldBorder、Packet、custom-ore-generator。1.7 舊附魔模擬要標成「委託人已接受正式移除」，不要列為 regression。
...
14. 做完整 config migration 與資料備份；Step 8 已收斂的 `cache.db`、`savedgames.db`、MySQL stats 與 scenario material 不代表全量 resource migration 已完成。
...
5. `RuntimeUtil.getTPS()` 已先補 Paper `getTPS()` / fallback stopgap，避免 Paper `1.21.11` 因 TPS placeholder 持續噴 `ClassNotFoundException` / NPE；Step 12 仍要把它移到正式 scoreboard metric adapter。
...
4. `gui.yml`、`items.yml`、`sounds.yml`、`biomes.yml` 的 alias 寫回與 menu button material 更新，跟 Step 12 menu/presentation 收斂一起處理。
