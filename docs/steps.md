...
- `Update-WonderlandUHC/src/main/java/org/mcwonderland/uhc/port/ScoreboardPort.java`：Step 18 才處理 scoreboard projection/view model 與 presentation 瘦身。
...
- `Update-WonderlandUHC/src/main/java/org/mcwonderland/uhc/menu/impl/PlayersMenu.java` 目前仍透過 Foundation `ItemCreator.of(CompMaterial)` 建立 player head；等 Step 18 menu adapter/view model 收斂時再處理。
- `Update-WonderlandUHC/src/main/java/org/mcwonderland/uhc/menu/ButtonLocalization.java` 目前被 Foundation `ButtonReturnBack.setMaterial(CompMaterial)` 簽名綁住；等 Step 18 替換或包住 Foundation menu API 時再處理。
- `Update-WonderlandUHC/src/main/java/org/mcwonderland/uhc/scenario/impl/ScenarioConfig.java` 目前同時被 Foundation `YamlConfig#getMaterial` 與 `ItemCreator.of(CompMaterial, ...)` 綁住；等 Step 8 設定/material migration、Step 14/15 scenario 收斂與 Step 18 scenario menu 顯示收斂時再處理。
...
本步驟處理 Step 17 未納入的展示與非核心入口，避免 command/menu/tools/scoreboard/practice 全部塞在同一步。
...
- Foundation menu / scoreboard 依賴已大幅收斂，剩餘項目能被 Step 21 最終 gate 明確追蹤。
...
5. 對 Step 5 到 Step 19 曾改動或降級的 legacy/platform/integration 行為要特別標註，例如 absorption、armor points、pickup exp control、death animation、custom exp orb、fast block set、large chest merge、WorldBorder、Packet、custom-ore-generator。1.7 舊附魔模擬要標成「委託人已接受正式移除」，不要列為 regression。
...
Step 5 與 Step 6 只負責讓 Foundation、DatouNMS 從主線退到 legacy 層；Step 11 到 Step 19 逐步替換核心入口、scenario、presentation 與 API 使用點。本步驟是發布前 gate，只做最後拔線、刪除依賴與完整驗證，不應承擔第一次大規模拆除。
...
6. 準備 README / DEVELOPMENT / 部署文件需要的最終依賴狀態，供 Step 22 發布文件使用。
...
- Step 20 的 1.16.5 對照 checklist 仍通過；移除 legacy 層不得造成舊版功能 regression，除非該差異已被使用者明確接受並記錄。
- A15 搜尋檢查與 A16 最終驗收清單都已更新為「Foundation / DatouNMS 完全移除」狀態。

## 22. 發布與文件

最後更新使用者文件與開發文件。此步驟必須在 Step 21 的最終依賴狀態確定後才做，避免發布文件描述和實際 runtime dependency 不一致。
...
6. 發布前確認 Step 20 測試對照與 Step 21 相容層移除 gate 都已通過。
...
18. Scoreboard / practice / 非核心 presentation 收斂。
19. Bukkit/Paper API 與剩餘 legacy adapter 清理。
20. 補測試與 1.16.5 / 1.21.11 功能對照檢核。
21. 在所有替代實作通過後，移除 Legacy Foundation / NMS 相容層。
22. 發布與文件更新。
...
5. `RuntimeUtil.getTPS()` 已先補 Paper `getTPS()` / fallback stopgap，避免 Paper `1.21.11` 因 TPS placeholder 持續噴 `ClassNotFoundException` / NPE；Step 18 仍要把它移到正式 scoreboard metric adapter。
...
4. `gui.yml`、`items.yml`、`sounds.yml`、`biomes.yml` 的 alias 寫回與 menu button material 更新，阻擋核心主持入口的部分先在 Step 11 處理，核心 menu resource 收斂在 Step 17，非核心 resource / menu presentation 收斂跟 Step 18 一起處理。
