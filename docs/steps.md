...
- 上述 runtime 流程遷移留到 Step 10；核心 presentation 入口瘦身留到 Step 16，scoreboard / practice / 非核心 presentation 收斂留到 Step 17。
...
- `Update-WonderlandUHC/src/main/java/org/mcwonderland/uhc/port/ScoreboardPort.java`：Step 17 才處理 scoreboard projection/view model 與 presentation 瘦身。
...
- 不重整 command、menu、tools、scoreboard 或 practice 的責任邊界；核心入口是 Step 16，scoreboard / practice / 非核心 presentation 是 Step 17。
...
- `Update-WonderlandUHC/src/main/java/org/mcwonderland/uhc/menu/impl/PlayersMenu.java` 目前仍透過 Foundation `ItemCreator.of(CompMaterial)` 建立 player head；等 Step 17 menu adapter/view model 收斂時再處理。
- `Update-WonderlandUHC/src/main/java/org/mcwonderland/uhc/menu/ButtonLocalization.java` 目前被 Foundation `ButtonReturnBack.setMaterial(CompMaterial)` 簽名綁住；等 Step 17 替換或包住 Foundation menu API 時再處理。
- `Update-WonderlandUHC/src/main/java/org/mcwonderland/uhc/scenario/impl/ScenarioConfig.java` 目前同時被 Foundation `YamlConfig#getMaterial` 與 `ItemCreator.of(CompMaterial, ...)` 綁住；等 Step 8 設定/material migration、Step 14/15 scenario 收斂與 Step 17 scenario menu 顯示收斂時再處理。
...
- 不處理 `gui.yml`、`items.yml`、`sounds.yml`、`biomes.yml` 的全量 alias 寫回；阻擋核心主持入口的 material 先在 Step 11 修，核心 menu resource 收斂在 Step 16，非核心 presentation resource 收斂留到 Step 17。
...
- 不把公告流程擴成新的主持指令功能。
- 不整理整體 command / menu 架構；這是 Step 16。
...
- 舊版預設 scenario 都有驗收結果；除非使用者明確接受，不能移除或永久降級。
- scenario 行為差異已分類為一致、可接受差異、版本限制或 regression。

## 16. 核心 Presentation 入口瘦身

Step 11 到 Step 13 只修「能不能用」與解除核心驗收阻塞；本步驟開始整理核心 command / menu / tools 的責任邊界。範圍限於主持與遊戲主流程入口，不含 scoreboard、practice 與非核心展示。

優先修改：

- `Update-WonderlandUHC/src/main/java/org/mcwonderland/uhc/command/uhc/**/*.java`
- `Update-WonderlandUHC/src/main/java/org/mcwonderland/uhc/command/impl/host/**/*.java`
- `Update-WonderlandUHC/src/main/java/org/mcwonderland/uhc/command/impl/game/**/*.java`
- `Update-WonderlandUHC/src/main/java/org/mcwonderland/uhc/menu/impl/host/**/*.java`
- `Update-WonderlandUHC/src/main/java/org/mcwonderland/uhc/tools/lobby/**/*.java`
- `Update-WonderlandUHC/src/main/java/org/mcwonderland/uhc/tools/spectator/**/*.java`
- `Update-WonderlandUHC/src/main/java/org/mcwonderland/uhc/tools/staff/**/*.java`
- `Update-WonderlandUHC/src/main/resources/gui.yml`
- `Update-WonderlandUHC/src/main/resources/items.yml`

要做的事：

1. `/uhc choose/start/stop/regen/tp/edit`、`/border`、白名單、staff、respawn、giveall 等核心主持工具只做參數解析、權限檢查、呼叫 use case、輸出結果。
2. Host menu 與 hotbar tools 只觸發 use case 或明確 application service，不直接改散落的 global state。
3. `gui.yml`、`items.yml` 中會影響核心入口的 material alias 與 button 設定完成 1.21.11 收斂。
4. Foundation command/menu/tool 仍可暫時存在，但核心入口的新邏輯不得再新增對 Foundation API 的直接耦合。

完成條件：

- 核心主持流程能以 command 或 GUI 完成，且 GUI 壞掉時仍有指令級替代路徑。
- 核心主持工具有指令級驗收清單，升級後行為與舊版等價或差異已被明確接受。
- 新增或修改的核心入口不再直接依賴 Foundation command/menu/tool API。

## 17. Scoreboard / Practice / 非核心 Presentation 收斂

本步驟處理 Step 16 未納入的展示與非核心入口，避免 command/menu/tools/scoreboard/practice 全部塞在同一步。
...
1. Scoreboard 改成讀取 projection/view model，不直接查全域 mutable state。
2. `RuntimeUtil.getTPS()` 的 Paper `getTPS()` / fallback stopgap 收斂成正式 scoreboard metric adapter。
...
- Foundation menu / scoreboard 依賴已大幅收斂，剩餘項目能被 Step 20 最終 gate 明確追蹤。

## 18. Bukkit/Paper API 與剩餘 Legacy Adapter 清理

當 Step 11 到 Step 17 已把核心入口、scenario 與 presentation 主要路徑拆開後，再集中清理剩餘 Bukkit/Paper API 與 legacy adapter。這一步是發布前移除相容層的前置整理，不是最後才第一次拆 Foundation / DatouNMS。
...
9. 對 `LegacyFoundationAdapter` / `LegacyDatouNmsAdapter` 剩餘呼叫分類：可立即替換、等 Step 20 拔除、或需明確保留為正式 platform service。

完成條件：

- 1.21.11 compile 通過。
- API 相容碼集中，不散落在 core/application。
- Step 20 移除相容層前的剩餘清單具體且可驗證。

## 19. 測試策略與 1.16 / 1.21 功能對照
...
5. 對 Step 5 到 Step 18 曾改動或降級的 legacy/platform/integration 行為要特別標註，例如 absorption、armor points、pickup exp control、death animation、custom exp orb、fast block set、large chest merge、WorldBorder、Packet、custom-ore-generator。1.7 舊附魔模擬要標成「委託人已接受正式移除」，不要列為 regression。
...
## 20. 最終移除 Legacy Foundation / NMS 相容層

Step 5 與 Step 6 只負責讓 Foundation、DatouNMS 從主線退到 legacy 層；Step 11 到 Step 18 逐步替換核心入口、scenario、presentation 與 API 使用點。本步驟是發布前 gate，只做最後拔線、刪除依賴與完整驗證，不應承擔第一次大規模拆除。
...
6. 準備 README / DEVELOPMENT / 部署文件需要的最終依賴狀態，供 Step 21 發布文件使用。
...
- Step 19 的 1.16.5 對照 checklist 仍通過；移除 legacy 層不得造成舊版功能 regression，除非該差異已被使用者明確接受並記錄。
- A15 搜尋檢查與 A16 最終驗收清單都已更新為「Foundation / DatouNMS 完全移除」狀態。

## 21. 發布與文件

最後更新使用者文件與開發文件。此步驟必須在 Step 20 的最終依賴狀態確定後才做，避免發布文件描述和實際 runtime dependency 不一致。
...
6. 發布前確認 Step 19 測試對照與 Step 20 相容層移除 gate 都已通過。
...
17. Scoreboard / practice / 非核心 presentation 收斂。
18. Bukkit/Paper API 與剩餘 legacy adapter 清理。
19. 補測試與 1.16.5 / 1.21.11 功能對照檢核。
20. 在所有替代實作通過後，移除 Legacy Foundation / NMS 相容層。
21. 發布與文件更新。
...
5. `RuntimeUtil.getTPS()` 已先補 Paper `getTPS()` / fallback stopgap，避免 Paper `1.21.11` 因 TPS placeholder 持續噴 `ClassNotFoundException` / NPE；Step 17 仍要把它移到正式 scoreboard metric adapter。
...
4. `gui.yml`、`items.yml`、`sounds.yml`、`biomes.yml` 的 alias 寫回與 menu button material 更新，阻擋核心主持入口的部分先在 Step 11 處理，核心 menu resource 收斂在 Step 16，非核心 resource / menu presentation 收斂跟 Step 17 一起處理。
