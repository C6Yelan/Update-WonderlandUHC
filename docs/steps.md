...
- `Update-WonderlandUHC/src/main/java/org/mcwonderland/uhc/port/ScoreboardPort.java`：Step 18 先驗收 scoreboard presentation 與 Paper `1.21.11` 相容性；只有實測需要時才建立最小 scoreboard helper / adapter，完整 port 化留到 Step 21 legacy 移除時判斷，Step 25 再做最終驗收。
...
- Foundation menu / scoreboard 依賴已完成分類；實際壞掉的項目已修正，未阻塞升級且可運作的項目能被 Step 21 legacy 移除明確追蹤，並在 Step 25 最終驗收。
...
10. `PlayerLoginEvent` 已完成 Paper `1.21.11` 替代流程研究，但不在 Step 19 直接重寫；它牽涉登入 gate、權限 bypass、白名單與 `UHCPlayer` 建立時機，需列入 Step 22 Login Gate Migration 專門處理，並在 Step 25 最終對照驗收。
...
Step 21 必須完成已知 Foundation / NMS legacy 移除、必要替代，或在本步驟內取得明確接受的行為差異。`PlayerLoginEvent` 登入 gate 已確認不是單純 legacy 拔除，而是涉及未來 Paper login API 與權限查詢策略的架構議題，正式移到 Step 22 專門處理。舊色碼 / message format boundary 交給 Step 23；若要完全移除 legacy config format，交給 Step 24。Step 25 只做最終對照檢查，除非檢查出 regression 才回到對應實作層修正。
...
6. 準備 README / DEVELOPMENT / 部署文件需要的最終依賴狀態，供 Step 26 發布文件使用。
...
- Step 19 延後的 deprecated 項目已被移除、替換，或改成非 legacy 的正式 platform/service 實作；`PlayerLoginEvent` 例外轉交 Step 22 以正式 login gate migration 處理，舊色碼 / message format boundary 轉交 Step 23，完整 legacy config format 移除轉交 Step 24。其他行為差異都必須在 Step 21 內完成決策與記錄，Step 25 只做最終對照驗證。
...
7. 更新 Step 24 / Step 26 需要的文件待辦，說明舊設定是否仍可使用、推薦的新格式，以及未來移除 legacy 相容時的 migration 路徑。
...
- README / DEVELOPMENT 的最終對外說明留到 Step 26，但本步驟已記錄訊息格式策略與維護邊界。
...
- `Update-WonderlandUHC/src/main/java/org/mcwonderland/uhc/platform/text/PluginText.java`
- 建議新增 `Update-WonderlandUHC/src/main/java/org/mcwonderland/uhc/platform/text/MessageFormatMigration.java`（若需要自動遷移）
...
2. 決定新格式策略：若採 MiniMessage-only，runtime parser 不再接受 legacy `&` / `§`；若採過渡期 dual-read，必須明確標記期限與移除條件。
3. 設計既有設定遷移方式：至少要有備份、可重跑性、失敗時不破壞原檔，以及人工修復說明。若不做自動 migrator，就不能宣稱 legacy 完全移除。
...
6. 補測試覆蓋 legacy -> 新格式遷移、MiniMessage parse、placeholder replacement、literal `<` / `>`、scoreboard line、item lore 與 click/hover text。
7. 針對既有伺服器資料夾測試 migration：確認舊 `messages.yml` / `items.yml` / `scoreboards.yml` 等可被備份並轉成新格式，且錯誤時有明確訊息。
...
- `src/main/java` / `src/test/java` 不再 import `LegacyComponentSerializer`。
- repo 預設 resources 不再使用 legacy `&` / `§` 色碼作為訊息格式；若有 literal `&` / `§`，必須是文件化的文字內容而非格式控制。
- 既有 legacy 設定有明確 migration 路徑，包含備份、失敗處理與手動修復方式。
- README / DEVELOPMENT 已說明新訊息格式與未來新增訊息的維護規則。
- 程式碼變更後依專案規則完成封裝、Paper `1.21.11` startup、`/uhc reload` 與訊息顯示抽樣測試。
- 若此步驟選擇保留任何 runtime legacy parser，必須明確記錄原因、期限與移除條件，且不能宣稱 legacy 完全消失。

## 25. 最終驗收：測試策略與 1.16 / 1.21 功能對照

Step 21 移除 legacy 相容層、完成 Step 22 Login Gate Migration、在 Step 23 收斂舊色碼 / message format boundary，並在 Step 24 完成下一代訊息格式 migration 後，必須用同一份 checklist 驗證升級後行為，而不是只靠人工印象或單次開服。這一步是發布前最終驗收，不再承擔新的 legacy 拔除、登入架構遷移或訊息格式 migration 工作；若驗收發現 regression，回到對應實作層修正後重新驗收。
...
2. 對同一份 checklist 分別在 Paper `1.16.5` 與 Step 24 完成後的 Paper `1.21.11` 執行，至少覆蓋：開服啟動、主要 command、host 設定、玩家加入、登入 gate（白名單、等待 host、滿員、遊戲中加入、bypass 權限）、隊伍、scenario、倒數、傳送、開局、死亡、觀戰、復活、掉落、經驗、邊界、世界生成、勝負判斷、結束流程、GUI、scoreboard、optional integrations。
3. 每個項目必須分類為「一致」、「可接受差異」、「版本限制造成的差異」、「升級造成 regression」、「原版既有問題」。
4. 所有「升級造成 regression」必須修正，或取得明確接受紀錄後才能進入發布步驟。
5. 對 Step 5 到 Step 24 曾改動或降級的 legacy/platform/integration 行為要特別標註，例如 absorption、armor points、pickup exp control、death animation、custom exp orb、fast block set、large chest merge、WorldBorder、Packet、custom-ore-generator、Foundation / DatouNMS 移除、登入 gate、team color、message format。1.7 舊附魔模擬要標成「委託人已接受正式移除」，不要列為 regression。

完成條件：

- core/application 不需要啟動 Bukkit 就能測。
- Paper smoke test 可驗證插件啟動與最小遊戲流程。
- Step 21 legacy 移除、Step 22 login gate migration、Step 23 message format boundary 收斂與 Step 24 message format modernization 後，1.16.5 / 1.21.11 功能對照檢核已完成，所有 regression 都已修正或被明確接受。
- A15 搜尋檢查、A16 最終驗收清單與發布前手動 checklist 都已通過。

## 26. 發布與文件

最後更新使用者文件與開發文件。此步驟必須在 Step 21 的最終依賴狀態、Step 22 的 login gate dependency / 行為策略、Step 23 的訊息格式邊界、Step 24 的訊息格式現代化結果與 Step 25 的最終驗收結果確定後才做，避免發布文件描述和實際 runtime dependency 不一致。
...
6. 發布前確認 Step 21 相容層移除、Step 22 Login Gate Migration、Step 23 Message Format Boundary、Step 24 Message Format Modernization 與 Step 25 最終新舊版對照驗收都已通過。
...
25. 補測試與 1.16.5 / 1.21.11 最終功能對照檢核。
26. 發布與文件更新。
