...
`scoreboard` 管理 sidebar、tab、below-name health 與 default theme loader。

`SidebarTheme.loadThemes()` 會從 `scoreboards.yml` 讀取 top-level theme；目前預設檔只保留 `Default`，host GUI 不再提供多風格切換。修改 scoreboard 時，除了 YAML，也要確認 placeholder 與 line 類別是否一致。
