...
| `wonderland.uhc.bypass.join.whitelist` | 白名單開啟時進入。 |
...
Chunky 是 softdepend，但對正式跑圖流程是必要外部插件。

也就是：

- 缺少 Chunky 時，WonderlandUHC 可以啟動。
- 缺少 Chunky 時，新的 UHC 世界無法完成正式預生成流程。
...
缺少 Chunky 時，missing adapter 在開始預生成時會丟出：
