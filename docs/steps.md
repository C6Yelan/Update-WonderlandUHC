...
- 不重整 static `Settings.*`、config migration 或 cache/db；這是 Step 8。舊 `populators.yml` 不再是 Step 8 migration 來源。
...
6. **DiscordSRV 完整保留但解除硬綁定**
   ...
   - 臨時 `/uhc announce` 已驗證 Discord 公告底層 sender 可送出，`Discord.Channel_Ids`、提及轉換與 `@everyone` active mention 可用。這代表 Step 7 的 DiscordSRV integration 能力已驗收；正式 GUI / Conversation 公告入口屬於 Step 12 的 GUI / Presentation 修復範圍，不能反向卡住 Step 7。
   ...
   - Step 12 接續事項：修復並驗證 `/uhc edit` 的 Discord 公告 GUI / Conversation 入口，確認它能使用同一套公告 sender、錯誤回報與 mention 行為；GUI 修復後移除臨時 `/uhc announce` 測試入口。
...
- DiscordSRV 沒安裝時只停用整合；安裝並啟用時，公告底層 sender、提及轉換、頻道設定與隊伍語音流程完整可用。GUI / Conversation 公告入口的修復與臨時 `/uhc announce` 移除屬於 Step 12，不作為 Step 7 阻塞項。
...
## 16. 移除 Legacy Foundation / NMS 相容層

Step 5 與 Step 6 只負責讓 Foundation、DatouNMS 從主線退到 legacy 層，避免升級初期一次重寫所有 command、menu、settings、scoreboard、scenario。完成 Step 8、Step 12、Step 13 與 A16 功能對照後，才進入本步驟，把過渡層本身移除。
...
6. 更新 README / DEVELOPMENT / 部署文件，刪除 Foundation 與 DatouNMS 作為建置或 runtime 條件的說法。
...
- `rg -n "DaTouNMS|datounms|NewerSpigotAPI|net\\.minecraft|org\\.bukkit\\.craftbukkit" Update-WonderlandUHC/src/main/java Update-WonderlandUHC/build.gradle` 為 0，除非是 Paper API 文件明確允許且封裝在 `platform/paper/` 的正式 adapter。
- `Update-WonderlandUHC` 可不依賴 `lib-foundation` repo 完成 clean build、shadow jar 與 Paper `1.21.11` server smoke test。
- Step 14 的 1.16.5 對照 checklist 仍通過；移除 legacy 層不得造成舊版功能 regression，除非該差異已被使用者明確接受並記錄。
...
8. 將 WorldBorder、PacketListenerAPI、DiscordSRV 改成 optional integration；舊 custom-ore-generator 整合移除，功能暫停到升級完成後重新設計。
...
16. 在所有替代實作通過後，移除 Legacy Foundation / NMS 相容層，讓主插件不再依賴 `lib-foundation`、DatouNMS 或 legacy NMS wrapper。
