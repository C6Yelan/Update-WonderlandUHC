...
9. 已知 Paper `1.21.11` 風險：`ScoreBoardUpdater -> ScoreLines -> StaffLines -> RuntimeUtil.getTPS()` 會嘗試反射舊 NMS TPS 欄位，實測會噴 `ClassNotFoundException: net/minecraft/server//MinecraftServer` 與連續 NPE。這不是 Chunky / WorldBorder 遷移造成的錯誤；Step 12 必須把 TPS 顯示改成 Paper 可維護 API、capability fallback，或在不可用時停用該 placeholder，避免 scoreboard async task 持續刷錯。
...
- 舊 WorldBorder plugin 的 `wb fill` 啟動行為移到 `integration/worldborder/LegacyWorldBorderPregenerationAdapter`；`ChunkFiller.fill()` 暫時保留成既有呼叫點的 facade。
- `WorldBorderFillFinishedEvent` 移到 `integration/worldborder/LegacyWorldBorderFillListener`，核心 `listener/` 不再直接依賴 `com.wimbli.WorldBorder` API。這不是新替代插件合格驗證；`Chunky` / `WorldBorder [Renew]` 實測仍是下一片。

第六片實測紀錄：

- 已在 Paper `1.21.11-130` 測試服安裝 `Chunky-Bukkit-1.4.40`，並暫時停用舊 `WorldBorder.jar` 為 `WorldBorder.jar.disabled`。啟動結果只載入 `Chunky` 與 `WonderlandUHC`，WonderlandUHC 正常 enable，dependency report 顯示 `WorldBorder: Disabled (Plugin is not hooked.)`，代表核心啟動不再需要舊 WorldBorder plugin。
...
- `ChunkFiller` 改由 `ChunkPregenerationAdapters` 選擇實作：優先使用 `Chunky`；若 `Chunky` 不存在但舊 `WorldBorder` plugin 仍可用，才退回 `LegacyWorldBorderPregenerationAdapter`；兩者都沒有時，預生成會給出明確錯誤，不會默默跳過。
...
- `ChunkPregenerationPort.startSquarePregeneration()` 明確接收預生成目標 radius，沿用舊 `getRadius(size) + 1` 外擴語意，讓 Chunky adapter 與 legacy WorldBorder fallback 都不需要依賴一般邊界流程裡的舊 `wb` 指令副作用。
- 舊 WorldBorder plugin 的 `wb set/wshape/fill/fill confirm` 已集中在 `integration/worldborder/LegacyWorldBorderPregenerationAdapter`，只會在 `Chunky` 不存在且舊 `WorldBorder` plugin 可用的 fallback 路徑執行。
- 本片完成後，`rg "dispatchCommand\\(.*wb"` 應只剩 `integration/worldborder` 的 legacy fallback。這表示舊 WorldBorder plugin 不再是一般邊界設定的隱性依賴。
...
5. `RuntimeUtil.getTPS()` 的舊 NMS 反射要移除或包成可降級的 scoreboard metric adapter；Paper `1.21.11` 上不能再因 TPS placeholder 持續噴 `ClassNotFoundException` / NPE。
