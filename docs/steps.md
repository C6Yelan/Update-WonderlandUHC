...
目前進度（2026-05-16）：Step 21 已建立 `step-21-legacy-removal` 分支並完成 21.1 的六個 DatouNMS 小切片：simple reads / TPS fallback（`1a07614`）、block placement（`0c53444`）、`ScenarioTimeBomb` large chest merge（`70c3d6c`）、death animation（`e1d3394`）、old enchant removal（`4be50ea`）與 exp pickup control（`144f00e`）。六個切片都已封裝、部署到 Paper `1.21.11` 測試服並由 `start.bat` 啟動到 `Done`。主流程 `LegacyDatouNmsAdapter.current()` call site 已清空；本輪已移除 `setupNms()`、`LegacyDatouNmsAdapter`、`PlatformCapabilities` 與 DatouNMS dependency，`src/main/java` / `build.gradle` 搜尋 gate 已無 DatouNMS / NMS 命中，封裝與 Paper startup 已通過，待 commit / push。
